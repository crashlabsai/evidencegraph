...
| LQ5 Integrity | supported | 2 claimed writes contradicted by the complete independent registry; runner consistency reported separately |
| LQ6 Coverage | supported | Declared mutation population; ambiguous and unmatched records are uncovered |
| LQ7 Refresh timing | ambiguous | Refresh minus claimed-B final event: -0.007s, declared clock bound ±1s |
| LQ8 Fixture access | supported | 1 successful cache reads after refresh; protected route denied access |
...
Validation: produced precision 1.0; confident errors 0. Full scoring and exclusions: [validation.json](lab-validation.json).
